...
void dtu_send(void)
{
   ...
   u16 crc;
	 psend=&send;  
	buf1[0] = base_id;  //本采集板地址
	buf1[1] = DEVICE_TYPE;	//设备类型
	buf1[2] = dtu_id; //节点id
	buf1[3] = 0x03;				//功能码
	buf1[4] = psend->length; //数据长度
//	buf1[3] = base_id; //
    	 while(dataCount<psend->length)//接收有效返回字节
				 {  
 				 	 buf1[5+dataCount] = psend->data[dataCount];
					 dataCount++;
				 }
	crc    = CRC_Check(buf1,3+psend->length);
	buf1[5+psend->length] = crc>>8;
	buf1[6+psend->length] = crc&0xff;
	USART_485_SendBytes(buf1,7+psend->length,1); 
